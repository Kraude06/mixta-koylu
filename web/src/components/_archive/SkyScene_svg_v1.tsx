// ARŞİV — SVG tabanlı SkyScene (köy silueti + gradyanlar), 2026-06-20
// Bu dosya yedek amaçlıdır. Aktif bileşen: ../SkyScene.tsx
  ...
  return (
    ...
    >
          ...
          {STARS.map((s, i) => (
            <circle key={i} cx={s.x} cy={s.y} r={s.r} fill={isHunter ? '#ffbbaa' : 'white'} opacity={0.75}>
              <animate attributeName="opacity"
                ...
                dur={`${2.2 + (i%5)*0.6}s`} begin={`${(i%5)*0.4}s`} repeatCount="indefinite"/>
            </circle>
          ))}
                  ...
                  <g opacity="0.36">
                    <g>
                      ...
                      <animateTransform attributeName="transform" type="rotate"
                        from={`0 ${sx} ${sy}`} to={`360 ${sx} ${sy}`} dur="48s" repeatCount="indefinite"/>
                    </g>
                  </g>
                )}
      ...
      <rect x="0" y="68%" width="100" height="5%"
        ...
        opacity={isNight ? 0.38 : 0.05} filter="url(#fog-blur)"/>
    </svg>
  );
}
